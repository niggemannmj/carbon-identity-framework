IF EXISTS (SELECT 1 FROM sysobjects WHERE id = object_id('WSO2_TOKEN_CLEANUP_SP'))
	DROP PROC WSO2_TOKEN_CLEANUP_SP
;
CREATE PROC WSO2_TOKEN_CLEANUP_SP
...
-- DECLARE VARIABLES
-- ------------------------------------------
DECLARE @batchSize UNSIGNED INT
...
SET @rowCount = 0
SET @enableLog = 1       -- ENABLE LOGGING [DEFAULT : FALSE]
...
-- ------------------------------------------------------
IF (EXISTS (SELECT 1 FROM tempdb.dbo.sysobjects WHERE id = object_id('tempdb.dbo.IDN_OAUTH2_ACCESS_TOKEN_BAK')))
BEGIN
    DROP TABLE tempdb.dbo.IDN_OAUTH2_ACCESS_TOKEN_BAK
END

IF (@backupTables = 1)
BEGIN
    SELECT * INTO tempdb.dbo.IDN_OAUTH2_ACCESS_TOKEN_BAK FROM dbo.IDN_OAUTH2_ACCESS_TOKEN
END
...
-- BACKUP IDN_OAUTH2_ACCESS_TOKEN TABLE
-- ------------------------------------------------------
IF (EXISTS (SELECT 1 FROM tempdb.dbo.sysobjects WHERE id = object_id('tempdb.dbo.IDN_OAUTH2_AUTHORIZATION_CODE_BAK')))
BEGIN
    DROP TABLE tempdb.dbo.IDN_OAUTH2_AUTHORIZATION_CODE_BAK
END

IF (@backupTables = 1)
BEGIN
    SELECT * INTO tempdb.dbo.IDN_OAUTH2_AUTHORIZATION_CODE_BAK FROM dbo.IDN_OAUTH2_AUTHORIZATION_CODE
END
...
WHILE @rowCount > 0
BEGIN
    DELETE dbo.IDN_OAUTH2_ACCESS_TOKEN WHERE TOKEN_STATE IN ('EXPIRED','INACTIVE','REVOKED') OR (TOKEN_STATE='ACTIVE' AND (@deleteTimeLimit > DATEADD(MINUTE, ((VALIDITY_PERIOD/1000)/60), TIME_CREATED) AND (@deleteTimeLimit > DATEADD(MINUTE, ((REFRESH_TOKEN_VALIDITY_PERIOD/1000)/60), REFRESH_TOKEN_TIME_CREATED))))
